...
cdef class ClassificationCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs,
		  ...
		  double WIGR_power,double fix,
                  DOUBLE_t[:] y_values): #WIGR
        ...
        self.WIGR_power = WIGR_power # alpha
        self.max_value = max(y_values)
        self.min_value = min(y_values)
        self.mode_value = 0.0
        self.EV_value = 0.0
        self.fix = fix
...
#############################################################################
#############################################################################
cdef class WIGR_EV(ClassificationCriterion):
    ...
    cdef double node_impurity(self) nogil:
        ...
        cdef SIZE_t k
        cdef SIZE_t c
        cdef double sum_v = 0.0  # new
        cdef double k_i = 0.0    #new
        cdef double max_v = 0.0 #new 
        cdef double max_p = 0.0 #new 
        cdef int n = 0 #new 
        cdef double sum_wp = 0.0  # new
        cdef double wp = 0.0  # new
        ...
        for k in range(self.n_outputs):
            ...
            for c in range(n_classes[k]):
                ...
                max_v += count_k * self.y_values[c]
                
            self.EV_value = max_v
            
            for c in range(n_classes[k]):
                sum_v += fabs(self.y_values[c]- max_v)**self.WIGR_power 
...
#                 if count_k > 0.0:
#                     count_k /= self.weighted_n_node_samples
#                     sum_wp += ((fabs(self.y_values[c] - max_v)**self.WIGR_power) / sum_v ) * count_k
# =============================================================================
            ...
            for c in range(n_classes[k]):
                ...
                if count_k > 0.0:
                    count_k /= self.weighted_n_node_samples
                    k_i = (fabs(self.y_values[c] - max_v)**self.WIGR_power) / sum_v # need to add power
                    entropy -= k_i * count_k * log(count_k)
# =============================================================================
#                     k_i = (fabs(self.y_values[c] - max_v)**self.WIGR_power) / sum_v # need to add power
#                     if k_i >0.0:
#                         wp = k_i * count_k / sum_wp
#                         entropy -= wp * log(wp)
# =============================================================================
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef SIZE_t k
        cdef SIZE_t c
        cdef double sum_v_left = 0.0  # new
        cdef double k_i_left = 0.0    #new
        cdef double max_v_left = 0.0 #new 
        cdef double max_p_left = 0.0 #new 
        cdef double sum_v_right = 0.0  # new
        cdef double k_i_right = 0.0    #new
        cdef double max_v_right = 0.0 #new 
        cdef double max_p_right = 0.0 #new 
        cdef int n_right = 0 
        cdef int n_left = 0 
        cdef double sum_wp_right = 0.0  # new
        cdef double sum_wp_left = 0.0  # new
        cdef double wp = 0.0  # new
        cdef double count_k_left  = 0.0  # new
        cdef double count_k_right  = 0.0  # new
        ...
        for k in range(self.n_outputs):
            ...
            if self.fix == 0.0:
                for c in range(n_classes[k]):
                        ...
                        max_v_right += count_k * self.y_values[c]
                          
                        count_k = sum_right[c]*self.y_values[c]
                        if count_k > 0.0:
                            count_k /= self.weighted_n_right
                            
                        max_v_left +=  count_k * self.y_values[c]
                    
            else:
                max_v_right = self.EV_value
                max_v_left = self.EV_value 
                    
            for c in range(n_classes[k]):
                
                sum_v_left += fabs(self.y_values[c]- max_v_left)**self.WIGR_power 
                sum_v_right += fabs(self.y_values[c]- max_v_right)**self.WIGR_power 
                    
# =============================================================================
#             for c in range(n_classes[k]):
#                 
#                 count_k_left = sum_left[c]
#                 if count_k_left > 0.0:
#                     count_k_left /= self.weighted_n_left
#                     sum_wp_left += ((fabs(self.y_values[c] - max_v_left)**self.WIGR_power) / sum_v_left ) * count_k_left
# 
#                 count_k_right = sum_right[c]
#                 if count_k_right > 0.0:
#                     count_k_right /= self.weighted_n_right
#                     sum_wp_right += ((fabs(self.y_values[c] - max_v_right)**self.WIGR_power) / sum_v_right ) * count_k_right          
# =============================================================================
                
            for c in range(n_classes[k]):

                k_i_left = (fabs(self.y_values[c] - max_v_left)**self.WIGR_power) / sum_v_left # need to add power 
                k_i_right = (fabs(self.y_values[c] - max_v_right)**self.WIGR_power) / sum_v_right # need to add power 

                count_k = sum_left[c]
                if count_k > 0.0 and k_i_left > 0.0 :
                    count_k /= self.weighted_n_left
                    entropy_left -= k_i_left * count_k *  log(count_k)

# =============================================================================
#                     wp = count_k * k_i_left / sum_wp_left
#                     entropy_left -= wp * log(wp)
# =============================================================================

                count_k = sum_right[c]
                if count_k > 0.0 and k_i_right > 0.0 :
                    count_k /= self.weighted_n_right
                    entropy_right -= k_i_right * count_k * log(count_k)

# =============================================================================
#                     wp = count_k * k_i_right / sum_wp_right
#                     entropy_right -= wp * log(wp)
# =============================================================================
...
#############################################################################
#############################################################################
cdef class WIGR_min(ClassificationCriterion):
    r"""Cross Entropy impurity criterion.
...
# =============================================================================
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        cdef double sum_v_left = 0.0  # new
        cdef double k_i_left = 0.0    #new
        cdef double max_v_left =  self.max_value #new 
        cdef double max_p_left = 0.0 #new 
        cdef double sum_v_right = 0.0  # new
        cdef double k_i_right = 0.0    #new
        cdef double max_v_right = self.max_value #new 
        cdef double max_p_right = 0.0 #new 
        cdef int n_right = 0 
        cdef int n_left = 0
        cdef double sum_wp_right = 0.0  # new
        ...
        for k in range(self.n_outputs):
            if self.fix ==0:
                ...
                for c in range(n_classes[k]):
                        count_k = sum_left[c]
                        if count_k > 0.0:
                            if  self.y_values[c] < max_v_left:
                                max_v_left = self.y_values[c]
                            
                        count_k = sum_right[c]
                        if count_k > 0.0:
                            if  self.y_values[c] < max_v_right:
                                max_v_right = self.y_values[c]
            else:
                max_v_left  = self.min_value 
                max_v_right = self.min_value 
...
#############################################################################
#############################################################################
cdef class WIGR_max(ClassificationCriterion):
    r"""Cross Entropy impurity criterion.
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            ...
            if self.fix ==0:                
                ...
                max_v_left  = self.max_value 
                max_v_right = self.max_value     
...
#############################################################################
#############################################################################
cdef class WIGR_mode(ClassificationCriterion):
    r"""Cross Entropy impurity criterion.
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            ...
            if self.fix == 0:
                ...
                max_v_right = max_v_right/n_right 
                max_v_left =  max_v_left/n_left  
            else:
                
                max_v_right =  self.mode_value
                max_v_left =  self.mode_value                 
